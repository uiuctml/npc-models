...
def counterfactual_cccp(outputs_decomposed_original, spn_joint, spn_marginal, spn_output_rows, spn_output_cols, labels_original, device):
    ...
    return (outputs_decomposed, _, _, _, _, _)
...
def counterfactual_gd(outputs_decomposed_original, spn_joint, spn_marginal, spn_output_rows, spn_output_cols, labels_original, device):
    ...
    return (utility.applySoftmaxDecomposed(outputs_decomposed), _, _, _, _, _)
...
def counterfactual_pgd_qp(outputs_decomposed_original, spn_joint, spn_marginal, spn_output_rows, spn_output_cols, labels_original, device):
    ...
    instance_count_corrected -= torch.sum(mask_qp_epsilon).item()
    for k in range(len(outputs_decomposed)):
    ...
    return (outputs_decomposed, instance_count_corrected, instance_count_incorrect, instance_count_qp_no_solution, instance_count_total, value_count_implausible)
...
def test(model_decomposed, spn_joint, spn_marginal, data_loader, device, batch_step):
    ...
    for (batch_index, (input, labels_decomposed, labels_original, input_file_paths)) in enumerate(data_loader):
        ...
        with torch.set_grad_enabled(True):
            (outputs_decomposed_counterfactual, instance_count_batch_corrected, instance_count_batch_incorrect, instance_count_batch_qp_no_solution, instance_count_batch_total, value_count_batch_implausible) = counterfactual_pgd_qp(outputs_decomposed_original, spn_joint, spn_marginal, spn_output_rows, spn_output_cols, labels_original, device)
